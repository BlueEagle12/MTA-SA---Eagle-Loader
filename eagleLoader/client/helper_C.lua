...
-- Is the current time within the given interval? Handles wrapping over midnight.
...
-- Timer to manage time-based streaming (e.g. night/day objects)
setTimer(function()
...
-- Mark an element for time-based streaming, if streamTimes exists for this id.
...
objectFlags = {
    ...
    {bit = 50, dec = 0, hex = "0x000000", name = "disable_collisions", description = "Disable collisions.", fun = setElementCollisionsEnabled, value = false}
}
...
-- Parse flag string(s) and return list
...
-- Convert flags on an attribute table to named keys
...
function prepIMGContainers(resourceName)
    if not fileExists(string.format(":%s/imgs/dff.img", resourceName)) then return end
